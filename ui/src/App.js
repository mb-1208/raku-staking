...
function App() {
  ...
  const getNftMedia = async (res, datas) => {
    let data = [];
    for (let i = 0; i < res.data.data.results.length; i++) {
      let reward = await getClaimableToken(
        contractStake,
        datas.nft_contract_id,
  ...
  const getStakeNft = async (stake) => {
    ...
    for (let data of stake) {
      ...
      // console.log(res);
      dataImage.push(await getNftMedia(res, data));
                  ...
                  <div className="bg-card p-3">
                    ...
                    <div class="progress mt-2">
                      <div
                        class="progress-bar"
                        role="progressbar"
                        style={{ color: 'white', background: '#d36597',width: `${(totalUserStaked / 555) * 100 }%` }}
